...
公开仓库只保留运行工作流所需的轻量模板文件。它不依赖私有的 `prompt/`、`skills/`、长篇提示词资料或历史赛题资料。
...
如果你本地还有 `prompt/`、`skills/`、`问题求解.md`、`可视化计划.md` 等资料，它们可以作为个人知识库辅助 AI，但不是这个公开工作流模板的必需依赖，也不会被提交到 GitHub。
...
公开模板中不包含 `prompt/` 和 `skills/`。工作流主入口统一使用 `AI_WORKFLOW.md`。
