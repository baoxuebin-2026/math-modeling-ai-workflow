...
def main() -> int:
    result = read_json("docs/results/q1_results.json", {})
    out_dir = ensure_dir("figures/q1")
    fig_path = out_dir / "q1_fig01_workflow_placeholder.png"
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                figure_note,
                "- 论文位置：问题一结果分析与可视化",
                f"- 当前状态：{result.get('status', '待补充')}",
                "- 后续要求：用正式模型输出替换占位图，并补充图题、变量、单位和关键发现。",
                "",
            ]
        ),
    )
