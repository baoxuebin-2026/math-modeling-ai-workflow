...
3. 在进入正式写作前，必须做一次“最终模型口径回收”：把 `docs/02_model_plan.md`、`docs/paper_materials.md`、`docs/04_result_summary.md`、`docs/06_validation_report.md` 与实际代码和结果对齐，删除或标注早期但未采用的模型说法、旧结果、旧参数、旧图表和旧敏感性分析。
4. 在引用任何图片前，必须按 `paper/workflow/figure_quality_gate.md` 对每张图逐张验收。图片文件存在、代码运行成功、PNG数量正确都不等于图可用。
5. AI 每次只写一个 `paper/sections/*.md` 文件。
6. 每写完一个章节，AI 必须按 `paper/workflow/section_gate.md` 做自检，生成对应 `paper/reviews/section_review_*.md`。
7. AI 把本章的主要内容、证据来源、待用户决策点列出来，等待用户确认或修改。
8. 用户确认后，AI 才进入下一章节。
9. 全部章节确认后，AI 按 `paper/workflow/merge_gate.md` 合并全文。
10. 合并后先做第一轮全文核对，修正文内引用、编号、符号、图表顺序和章节衔接。
11. 再按 `paper/workflow/abstract_gate.md` 重写标题、摘要和关键词。
12. 最后做第二轮全文核对，确认摘要中的每个数值、模型名、结论都能在正文找到。
