...
description: 专门用于全国大学生数学建模竞赛（CUMCM）本科组 C 题的选题判断、数据分析、模型设计、算法求解、创新设计、可视化与 Markdown 论文草稿写作。用于用户要求解决或复盘国赛 C 题、根据附件建立 C 题模型、设计 72 小时方案、撰写/修改 C 题 Markdown 论文草稿或评估 C 题论文竞争力时。只服务 C 题；不要将 A/B/D/E 题套路机械迁入。
---
...
- 论文阶段只生成 Markdown 文本，不生成 Word、PDF 或其他排版文件；
- 保持标题层级、图题、表题、公式编号和正文引用清晰一致；
- 公式使用可迁移的 Markdown 数学表达，图表使用真实生成文件的相对路径；
- 完成 `paper/drafts/final_paper_draft.md` 后停止，后续排版由用户自行完成。
...
6. 第二天完成主模型、创新与验证。
7. 第三天集中做叙事、Markdown 论文草稿、图表、参考文献、附录与复现。
8. 使用 `scripts/validate_project.py` 自动检查结构，再人工做评委视角终检。

## 14. 交付工程

生成论文 Markdown 草稿时，至少交付：
