...
- **light local mode**: use for paragraph rewrites, chart choices, abstract style notes, exploratory data/code inspection, or Markdown readability fixes. Label outputs as `local advice`, `diagnostic-only`, or `paper-ready gate required`.
...
- Markdown paper structure, section content, figure/table placement, and final consistency checks: `math-templates` plus `math-consistency` and `math-review`.
