...
Downstream rule: paper-ready code outputs, final abstract claims, and final Word submission work should not proceed from a modeling route whose quality class is not `national-first-candidate`. Exploratory/diagnostic code, abstract style coaching, and local Word layout fixes may proceed when clearly labeled as local advice or `diagnostic-only`; they must not promote claims into final paper language.
